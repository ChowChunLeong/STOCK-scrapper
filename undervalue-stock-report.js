...
(async () => {
  ...
  // Filter in screener
    await page.type('input[name="max_pe"]', '5');
    await page.$eval('input[name="rsi"][value="neutral"]', check => check.checked = true)
    await page.select('select[name="profitable_type"]', "quarters")
    await page.select('select[name="profitable_years"]', "3")
...
//! scrap pick a stock for analyiz epf kwsp 成本区